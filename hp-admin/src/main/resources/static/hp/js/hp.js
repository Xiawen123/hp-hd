    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                _singleSelect = $.common.isEmpty(options.singleSelect) ? false : options.singleSelect;
                $('#bootstrap-table').bootstrapTable({
                    ...
                    singleSelect: _singleSelect,                        // 是否是单选
    	...
    	modal: {
            ...
            openOptions: function (options) {
                ...
                var _height = $.common.isEmpty(options.height) ? ($(window).height()) : options.height;
                layer.open({
                    type: 2,
            		maxmin: true,
                    shade: 0.3,
        ...
        operate: {
            ...
            // 选手审核
            audit: function(id) {
                $.modal.confirm("是否通过审核？", function() {
                    var url = $.common.isEmpty(id) ? $.table._option.auditUrl : $.table._option.auditUrl.replace("{id}", id);
                    var data = { "id": id };
                    $.operate.submit(url, "post", "json", data);
                });
            },
            // 班级结课
            change: function(id) {
                $.modal.confirm("确定结课该" + $.table._option.modalName + "吗？", function() {
                    var url = $.table._option.changeUrl;
                    var data = { "ids": id };
                    $.operate.submit(url, "post", "json", data);
                });
            },
